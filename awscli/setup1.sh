...
# VARIABLES
VPC_NAME="vpc-mensagl-2025-diegocastillo"
...
# Crear subredes
SUBNET_PUBLIC_1=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.205.1.0/24" --availability-zone "$REGION"a \
  --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=Public1}]' \
  --query 'Subnet.SubnetId' --output text)
echo "Subred pública 1 creada: $SUBNET_PUBLIC_1"

SUBNET_PUBLIC_2=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.205.2.0/24" --availability-zone "$REGION"b \
  --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=Public2}]' \
  --query 'Subnet.SubnetId' --output text)
echo "Subred pública 2 creada: $SUBNET_PUBLIC_2"

SUBNET_PRIVATE_1=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.205.3.0/24" --availability-zone "$REGION"a \
  --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=Private1}]' \
  --query 'Subnet.SubnetId' --output text)
echo "Subred privada 1 creada: $SUBNET_PRIVATE_1"

SUBNET_PRIVATE_2=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block "10.205.4.0/24" --availability-zone "$REGION"b \
...
INSTANCE_EJABBERD=$(aws ec2 run-instances \
  ...
  --private-ip-address 10.205.1.10 \
  --security-group-ids $SG_EJABBERD \
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=equipo3diegoc-ejabberd}]' \
...
INSTANCE_POSTGRES=$(aws ec2 run-instances \
  ...
  --private-ip-address 10.205.3.100 \
  --security-group-ids $SG_POSTGRESQL \
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=equipo3diegoc-postgresql}]' \
  --query 'Instances[0].InstanceId' --output text)
...
INSTANCE_CMS=$(aws ec2 run-instances \
  ...
  --private-ip-address 10.205.2.10 \
  --security-group-ids $SG_CMS \
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=equipo3diegoc-cms}]' \
  --associate-public-ip-address \
  --query 'Instances[0].InstanceId' --output text)
...
INSTANCE_MYSQL=$(aws ec2 run-instances \
  ...
  --private-ip-address 10.205.4.100 \
  --security-group-ids $SG_MYSQL \
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=equipo3diegoc-mysql}]' \
  --query 'Instances[0].InstanceId' --output text)
