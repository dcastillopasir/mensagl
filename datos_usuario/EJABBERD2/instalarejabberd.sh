...
# Añadimos la configuración de mod_http_upload
echo "Añadiendo configuración para mod_http_upload..."
sudo sed -i '/mod_http_upload:/a \ \   docroot: "/var/lib/ejabberd/upload"\n    put_url: "https://dcastillop.local/upload"\n    get_url: "https://dcastillop.local/upload"\n    custom_headers:\n      "Access-Control-Allow-Origin": "*"\n      "Access-Control-Allow-Methods": "GET,HEAD,PUT,OPTIONS"\n      "Access-Control-Allow-Headers": "Content-Type"' "$CONFIG_FILE"

echo "Configuración de mod_http_upload añadida a ejabberd.yml."

# Creamos el directorio de carga y configuramos los permisos
echo "Creando el directorio /var/lib/ejabberd/upload..."
sudo mkdir -p /var/lib/ejabberd/upload

# Establecemos los permisos adecuados
echo "Estableciendo permisos para /var/lib/ejabberd/upload..."
sudo chown ejabberd:ejabberd /var/lib/ejabberd/upload
sudo chmod 755 /var/lib/ejabberd/upload
